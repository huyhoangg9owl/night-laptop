    ...
    <div class="mt-8">
        <div class="flex items-center justify-between pb-6">
            ...
            <p class="text-lg font-medium leading-8 text-black dark:text-white">
                <?= number_format(array_sum(array_map(function ($order) use ($Product) {
                    $product = $Product->getProductById($order['product_id']);
                    return $order['quantity'] * $product['price'];
                }, $cart)), 0, "", ",") ?> đ
            </p>
        </div>
        <form action="/services/account/cart/process" method="post" class="gap-0 max-xl:grid max-xl:grid-cols-2 max-xl:grid-rows-2 max-xl:gap-x-8">
            ...
            <button class="order-3 col-span-1 mt-4 h-fit w-full cursor-not-allowed rounded-lg bg-black px-4 py-2 text-center text-sm font-semibold text-white transition-all duration-500 hover:bg-black/80 max-xl:mt-0 max-xl:py-4" type="button" name="discount">
                Áp dụng
            </button>
            ...
            <button class="order-4 col-span-1 h-fit w-full rounded-xl bg-indigo-600 px-6 py-3 text-center text-lg font-semibold text-white transition-all duration-500 hover:bg-indigo-700">
                Thanh toán
            </button>
        </form>
    </div>
