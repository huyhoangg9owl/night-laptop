<tr>
    <td class="flex items-center gap-4 py-6">
        <img src="<?= $order['image_url'] ?? "/public/favicon.ico" ?>" alt="<?= $order['name'] ?>" class="h-20 w-20 rounded-lg object-cover">
        <div>
            <h3 class="text-lg font-semibold text-black dark:text-white"><?= $order['name'] ?></h3>
            <p class="text-base font-normal text-gray-400 dark:text-gray-400"><?= $order['category'] ?></p>
        </div>
    </td>
    ...
    <td class="text-base font-normal text-gray-600 dark:text-white">
        <input type="hidden" name="product[<?= $index ?>][id]" value="<?= $order['id'] ?>">
        <div class="w-fit rounded-lg border border-gray-200 bg-white dark:border-neutral-700 dark:bg-neutral-900" data-hs-input-number='{"min": 0}'>
    ...
    <td class="text-base font-normal text-gray-600 dark:text-white">
        <?= number_format($order['quantity'] * $order['price'], 0, "", ",") ?> đ
    </td>
</tr>