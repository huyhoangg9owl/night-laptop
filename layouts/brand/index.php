...
<main class="w-full min-h-dvh relative p-4 container mx-auto">
    <form method="get" class="flex flex-wrap -mx-4 mt-8">
        <?php require_once ROOT_PATH . "/layouts/brand/filter/index.php" ?>
        <div class="w-full xl:w-3/4 px-4">
            <?php
            require_once ROOT_PATH . "/layouts/brand/products-table/quick_filter/index.php";
            ?>
            <div class="bg-white p-4 rounded-lg shadow-lg dark:bg-black/40">
                <?php
                require_once ROOT_PATH . "/layouts/brand/products-table/view_mode/' . $view_mode . '.php";
                require_once ROOT_PATH . "/layouts/brand/pagination.php";
                ?>
            </div>
        </div>
    </form>
